...
I will be using a random forest for the prediction model. Since random forests tend to overfit I am  planning for separate cross validation. Therefore, I am creating a random row index "subset" for  splitting the training set into a training and a validation set.
...
With a out-of-bucket estimated error rate of only 0.83% this model fits quite well. However, let's confirm the fit through cross-validation.
...
Cross validation on a random set of 40% of the testing data shows that in 95% of the time the model accuracy is between 98.9% and 99.1%. That's indeed a very good fit and I feel comfortable to predict the 20 test cases.
